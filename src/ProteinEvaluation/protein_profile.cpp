...
bool ProteinProfile::CalculateProfiles(int nFlag, bool bVerbose)
{
	...
	if (nFlag & AlgnID)
	{
		vecCalcFuncs.push_back([=] {
			Calculate_Alignment_Profile(nFlag & FragID,
										_dFrag_Score_Cutoff,
												 ...
												 double dGap_Penalty, int nMin_Frag)
{
	...
	for (auto &[score, seq1, seq2, atom_dist] : _vecTupleAlignments)
	{
		...
		for (int i = 1; i <= align_len; ++i)
			sum_dist[i] = (atom_dist[i] == GAP_NUMBER ? _dGap_Penalty : atom_dist[i]) + sum_dist[i - 1];
	...
	for (int i = 0; i < 20; ++i)
		for (int j = 0; j < 7; ++j)
			if (solvent_class_total[j] && _aAA_Total_Count[i])
				_aSolvent_Profile[i][j] = (double)_aSolvent_AA_Count[i][j] / (solvent_class_total[j] * (_aAA_Total_Count[i] / (double)nAA_Total));
	bSolvent_Rdy = true;
}
	...
	for (int i = 0; i < 20; ++i)
		for (int j = 0; j < 7; ++j)
			if (sec_class_total[j] && _aAA_Total_Count[i])
				_aSec_Profile[i][j] = (double)_aSec_AA_Count[i][j] / (sec_class_total[j] * (_aAA_Total_Count[i] / (double)nAA_Total));
	...
	if (nFlag & FamID)
	{
		...
		if (utility::FileExists(fFamily))
		{
			...
			while (std::getline(inFile, HEAD))
				if (HEAD != "")
				{
					auto split = utility::split(HEAD, "\\s");
					HEAD = utility::split(split[0], "\\/").back();
